...
export class CustomersService {
  ...
  try {
    // Make sure the record exists (and get tenantId for duplicate check)
    ...
    // ---- Build Prisma.CustomerUpdateInput from loose body ----
    ...
    // 🔁 Addresses replace strategy (optional)
    // NOTE: In nested writes, DO NOT filter by foreign key; use {}.
    if (Array.isArray(body?.addresses)) {
      ...
      updateData.addresses =
        arr.length === 0
          ? { deleteMany: {} } // remove all existing addresses for this customer
          : {
    ...
    // Unique fallback for race conditions
    if (error?.code === 'P2002' && error?.meta?.target?.includes('email')) {
      throw new BadRequestException(
        'Another customer with this email already exists for this tenant',
      );
