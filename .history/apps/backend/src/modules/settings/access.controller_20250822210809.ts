...
export class AccessController {
  ...
  /** Require the strict permission for managing Access Control. */
  private async ensureCanManage(req: any, tenantId: number) {
    // Prefer an already-attached permission list, else resolve on the fly
    const reqPerms: string[] | undefined = req?.permissions;
    let perms: string[] = Array.isArray(reqPerms) ? reqPerms : [];

    if (!perms.length) {
      const userId = Number(req?.user?.userId ?? req?.user?.id);
      if (!userId) throw new ForbiddenException('Not authenticated');
      perms = await this.perms.resolveForUser(userId, tenantId);
    }

    if (!perms.includes('settings.access.manage')) {
      throw new ForbiddenException('Requires settings.access.manage');
    }
  }
  ...
  async getOverrides(@Req() req: any, @Param('userId') userId: string) {
    ...
    await this.ensureCanManage(req, tenantId); // 🔒 enforce permission
