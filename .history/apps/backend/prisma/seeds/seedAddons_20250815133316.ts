...
import { PrismaClient, Amenity, RoomType, SeasonalRate, Booking, PosOrder, License } from '@prisma/client';
...
async function main() {
  ...
  for (const data of amenitiesData) {
    let amenity: Amenity | null = await prisma.amenity.findUnique({ where: { name: data.name } });
    if (!amenity) {
  ...
  for (const name of licenseNames) {
    let license: License | null = await prisma.license.findUnique({ where: { name } });
    if (!license) {
