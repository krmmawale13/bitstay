...
const NAV_ITEMS: Array<{ path: string; required?: string[] }> = [
  ...
  { path: "/bar", required: ["pos.use", "bars.read"] }, // whichever you prefer
  { path: "/inventory", required: ["inventory.read"] },
  { path: "/reports", required: ["reports.view"] },
  { path: "/settings/access", required: ["dashboard.view"] }, // or ADMIN-only later
];
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      try {
        ...
        // ---- SMART LANDING / ACCESS CORRECTION ----
        // If user is on "/" → go to first allowed page
        ...
        // If user opened a page they can't view (e.g., /dashboard without dashboard.view),
        // push them to the first allowed page instead of showing empty/locked content.
