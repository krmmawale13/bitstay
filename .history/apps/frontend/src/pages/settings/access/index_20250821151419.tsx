...
export default function AccessControlPage() {
  ...
  // --- load users (tenant-scoped). If GET /users not implemented yet, we degrade gracefully.
  useEffect(() => {
    (async () => {
      setLoadingUsers(true);
      try {
        // Expected backend shape (recommended): [{ id, user:{id,name,email,isActive}, role }]
          ...
          {selected && (
            <div className="space-y-6">
              ...
              <Section title="Grant (Add)">
                <PermGrid list={ov.add ?? []} onToggle={(p) => toggle(p, "add")} />
              </Section>

              <Section title="Revoke (Remove)">
                <PermGrid list={ov.remove ?? []} onToggle={(p) => toggle(p, "remove")} />
              </Section>
            </div>
          )}
        </main>
...
/* ---------- small local components (keep page self-contained) ---------- */
