...
// ✅ Add interceptor to include Authorization token in every request
axios.interceptors.request.use((config) => {
  const token = localStorage.getItem("accessToken"); // token after login
  if (token) {
...
export async function getCustomers(
  ...
  const url = tenantId
    ? `${CUSTOMERS_URL}?tenantId=${tenantId}`
    : CUSTOMERS_URL;
