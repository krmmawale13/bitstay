...
import {
  setCurrentUser,
  type CurrentUser,
  finalizeLoginSession,   // ⬅️ new
  clearPermissions,       // ⬅️ new
} from "@/lib/auth";
...
export async function login(email: string, password: string): Promise<LoginResponse> {
  ...
  // 4) 🔑 hydrate effective permissions for this user+tenant
  //    - seeds tenant if missing
  //    - fetches /auth/permissions
  //    - writes to localStorage.permissions
  //    - dispatches "permissions:updated"
...
export function logout() {
  ...
  localStorage.removeItem("activeTenantId");
  localStorage.removeItem("tenantId");
  clearPermissions(); // 🔒 also clears localStorage.permissions + dispatches "permissions:updated"
}
