...
export default function Topbar({ toggleSidebar }: TopbarProps) {
  ...
  return (
    <motion.header
      ...
      className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 shadow px-6 py-3 relative"
    >
      {/* Top Row: Branding */}
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xl font-bold text-teal-600 dark:text-teal-400">BitStay</span>
        </div>
        <div className="text-xs text-gray-500 dark:text-gray-400">CRM Dashboard</div>
      </div>

      {/* Bottom Row: Search + Actions */}
      <div className="flex items-center justify-between">
        {/* Search */}
        <div className="flex items-center bg-gray-100 dark:bg-gray-800 rounded-lg px-3 py-1 w-64 focus-within:ring-2 focus-within:ring-teal-400">
        ...
        <div className="flex items-center gap-4">
          {/* Theme Toggle */}
          <motion.div whileHover={{ scale: 1.15 }} onClick={() => setDarkMode(!darkMode)} className="cursor-pointer">
            {darkMode ? <Sun className="w-6 h-6 text-yellow-400" /> : <Moon className="w-6 h-6 text-gray-600 dark:text-gray-300" />}
          </motion.div>
