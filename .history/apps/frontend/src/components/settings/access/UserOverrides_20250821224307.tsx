...
// 🔒 Canonical: tenant ONLY from login flow (localStorage)
...
export default function UserOverrides() {
  ...
  // Fetch users (requires tenant from login)
  ...
  const loadOverrides = async (u: UserLite) => {
    ...
    try {
      const tenantId = getTenantId();
      if (!tenantId) throw new Error("Select a tenant first (login/topbar).");
