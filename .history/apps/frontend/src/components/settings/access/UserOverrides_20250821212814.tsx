...
import { getCurrentUser } from "@/lib/auth"; // ⬅️ add
...
/** ⬇️ MUST: get tenantId or initialize it from currentUser */
function getOrInitTenantId(): string {
  ...
  return ""; // still missing -> caller should handle
}
...
export default function UserOverrides() {
  ...
  // Fetch users (ENSURE TENANT FIRST)
  const fetchUsers = async () => {
    setLoadingUsers(true);
    setErrorUsers(null);
    try {
      const tenantId = getOrInitTenantId();
      if (!tenantId) throw new Error("Select a tenant first.");

      const { data } = await http.get<UserLite[]>("/users", {
        headers: { "x-tenant-id": tenantId },
        params: { tenantId }, // bonus: guard also accepts query
      });
