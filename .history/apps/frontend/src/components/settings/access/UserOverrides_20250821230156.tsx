...
// 🔒 Canonical: tenant ONLY from login/topbar (localStorage)
function getTenantId(): string | null {
  if (typeof window === "undefined") return null;
  const a = localStorage.getItem("activeTenantId");
  if (a) return a;
  const b = localStorage.getItem("tenantId");
  return b ? b : null;
}
...
export default function UserOverrides() {
  ...
  const fetchUsers = async (tid?: string | null) => {
      ...
      <main className="flex-1 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/70 backdrop-blur-xl p-4">
        {!selected ? (
          <div className="text-sm text-muted-foreground">
            {errorUsers ? "Fix tenant selection to proceed." : "Select a user to view and edit overrides."}
          </div>
        ) : (
