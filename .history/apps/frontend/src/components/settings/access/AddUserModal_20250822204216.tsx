...
export default function AddUserModal({ open, onClose, onCreated }: Props) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!tenantId) {
      const msg = "Select a tenant first.";
      setError(msg);
      window.__toast?.push?.({ type: "info", message: msg, description: "Choose the active tenant from the top bar." });
      return;
    }

    const cu = getCurrentUser();
    if (cu?.role !== "ADMIN") {
      const msg = "Only ADMIN can add users.";
      setError(msg);
      window.__toast?.push?.({ type: "error", message: "Access denied", description: msg });
      return;
    }
    ...
    if (!isValidEmail) {
      ...
      setError(msg);
      window.__toast?.push?.({ type: "info", message: "Invalid email", description: msg });
      return;
    }
    ...
    try {
      ...
      window.__toast?.push?.({ type: "success", message: "User created", description: data?.user?.email ?? email });
      onCreated?.(data);
    } catch (err: any) {
      const msg =
      ...
      setError(msg);
      window.__toast?.push?.({ type: "error", message: "Create failed", description: msg });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const copy = async (txt: string) => {
    try {
      ...
      setCopied(true);
      window.__toast?.push?.({ type: "success", message: "Temporary password copied" });
      setTimeout(() => setCopied(false), 1200);
    } catch {
      window.__toast?.push?.({ type: "error", message: "Copy failed", description: "Could not copy to clipboard." });
    }
  };
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1000] flex items-center justify-center">
      ...
      >
        ...
        {/* Error banner (kept, plus we now also toast) */}
        {error && (
