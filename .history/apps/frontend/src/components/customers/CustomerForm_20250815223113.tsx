...
export async function getCustomers(): Promise<Customer[]> {
  const res = await axios.get<Customer[]>(CUSTOMERS_URL);
  return res.data;
}

// GET ONE
export async function getCustomerById(id: string | number): Promise<Customer | null> {
  const res = await axios.get<Customer | null>(`${CUSTOMERS_URL}/${id}`);
...
export async function deleteCustomer(id: string | number): Promise<{ success: boolean }> {
  const res = await axios.delete<{ success: boolean }>(`${CUSTOMERS_URL}/${id}`);
  return res.data;
}
