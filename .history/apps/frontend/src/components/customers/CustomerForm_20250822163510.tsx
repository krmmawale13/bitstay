...
export default function CustomerForm({ customerId, onSuccess, onCancel }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setGlobalError(e?.message || "Failed to load customer");
      } finally {
  ...
  const validate = () => {
    ...
    if (form.email && form.email.trim() && !emailPattern.test(form.email)) {
      e.email = "Invalid email";
    }

    if (form.phone && form.phone.trim() && !phonePattern.test(form.phone)) {
    ...
    // Pincode basic
    if (A.pincode && A.pincode.trim() && !/^[0-9A-Za-z\- ]{3,10}$/.test(A.pincode)) {
