...
const has = (set: Set<string>, k: string) => set.has(k);
...
export default function CustomerTable() {
  ...
  // RBAC (read from localStorage populated by /auth/permissions)
  const perms = useMemo(() => new Set(readJSON<string[]>("permissions", [])), []);
  const canRead = has(perms, "customers.read");
  const canWrite = has(perms, "customers.write");

  async function loadCustomers() {
    if (!canRead) return; // respect RBAC
    setLoading(true);
    try {
  ...
  async function handleDelete(id: number, name?: string | null) {
    if (!canWrite) {
      (window as any).__toast?.push?.({ type: "error", message: "Not allowed", description: "You can't delete customers." });
      return;
    }
    const ok = confirm(
      `Delete customer${name ? ` "${name}"` : ""}? This action cannot be undone.`
    );
    if (!ok) return;
    try {
      setDeletingId(id);
      await deleteCustomer(id);
      (window as any).__toast?.push?.({ type: "success", message: "Customer deleted" });
      await loadCustomers();
    } catch (err: any) {
      (window as any).__toast?.push?.({
        type: "error",
  ...
  function handleEdit(customer: Customer) {
    if (!canWrite) {
      (window as any).__toast?.push?.({ type: "error", message: "Not allowed", description: "You can't edit customers." });
      return;
    }
  ...
  function handleCreate() {
    if (!canWrite) {
      (window as any).__toast?.push?.({ type: "error", message: "Not allowed", description: "You can't create customers." });
      return;
    }
  ...
  useEffect(() => {
    ...
    window.addEventListener("app:refresh-customers", onRefresh as any);
    return () => window.removeEventListener("app:refresh-customers", onRefresh as any);
  ...
  // RBAC: if no read, render a soft lock screen
  if (!canRead) {
    return (
      <div className="p-6">
        <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-slate-900/60 p-8 text-center">
          ...
          <h3 className="text-base font-semibold text-slate-900 dark:text-slate-100">No access to Customers</h3>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            Ask an admin to grant <code className="font-mono">customers.read</code> permission.
          </p>
        </div>
      </div>
    );
      ...
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title={canWrite ? "Add new customer" : "You don't have write access"}
          >
            <Plus size={16} />
            New
          </button>
              ...
              {filtered.length > 0 ? (
                    ...
                    <Td className="text-center">
                      <div className="flex items-center justify-center gap-2">
                        <button
                          onClick={() => {
                            if (!canRead) {
                              (window as any).__toast?.push?.({ type: "error", message: "Not allowed" });
                              return;
                            }
                            router.push(`/customers/${c.id}/customerdashboard`);
                          }}
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1 rounded-lg px-3 py-1.5 text-sm font-medium text-white
                                     bg-amber-500/90 hover:bg-amber-500 transition shadow-sm disabled:opacity-50"
                          title={canWrite ? "Edit" : "No write access"}
                        >
                          <Pencil size={16} />
                          Edit
                        </button>
                        <button
                          ...
                          disabled={!canWrite || deletingId === c.id}
                          className="inline-flex items-center gap-1 rounded-lg px-3 py-1.5 text-sm font-medium text-white
                                     bg-rose-600 hover:bg-rose-500 disabled:opacity-50 transition shadow-sm"
                          title={canWrite ? "Delete" : "No write access"}
                        >
                          <Trash2 size={16} />
                ...
                <tr>
                  <Td colSpan={7} className="py-10">
                    <EmptyState onCreate={handleCreate} disabled={!canWrite} />
                  </Td>
                </tr>
              )}
            </tbody>
        ...
        {showForm && (
          <div className="rounded-2xl border border-slate-200 bg-white/70 p-4 shadow-sm backdrop-blur dark:border-slate-800 dark:bg-slate-900/60">
            ...
            <CustomerForm
              ...
              onSuccess={() => {
                setShowForm(false);
                // let other widgets refresh themselves if needed
                try { window.dispatchEvent(new CustomEvent("app:refresh-customers")); } catch {}
                loadCustomers();
              }}
    ...
    <div className="mx-auto max-w-md text-center">
      ...
      <button
        ...
        className="mt-4 inline-flex items-center gap-2 rounded-xl px-4 py-2 font-medium text-white
                   bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500
                   shadow-sm active:scale-[0.98] disabled:opacity-50"
        title={disabled ? "No write access" : "New Customer"}
      >
        <Plus size={16} />
        New Customer
      </button>
