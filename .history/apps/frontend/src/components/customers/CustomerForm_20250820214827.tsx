...
/**
 * CLEAN SYNC with backend where Address.state is a RELATION to State.
 * Frontend now sends `stateCode` (e.g., "MH") so the backend can connect/connectOrCreate.
 */
...
type Address0 = {
  ...
  line2?: string;
  city?: string;
  stateCode?: string; // <-- send code like "MH"; backend maps to relation
  zip?: string;
};

// API address (when loading existing customer)
// state may be string (legacy) or object with code/name
// We'll prefer `code`, fall back to empty string.
type ApiAddress = {
  line1?: string | null;
  line2?: string | null;
  city?: string | null;
...
export default function CustomerForm({ customerId, onSuccess, onCancel }: Props) {
  const [form, setForm] = useState<{
    ...
    email: "",
    phone: "",
    address0: { line1: "", line2: "", city: "", stateCode: "", zip: "" },
  });
  ...
  useEffect(() => {
    ...
    getCustomerById(customerId)
      .then((data: Customer | null) => {
        ...
        // Derive a stateCode from API value
        let stateCode = "";
        if (typeof first.state === "string") {
          // If API returned legacy string like "Maharashtra", we cannot reliably map to code.
          // Keep empty and let user fill code; or you may implement a name->code map if desired.
          stateCode = "";
        } else if (first.state && typeof first.state === "object") {
          stateCode = (first.state.code ?? "").toString();
        }
  ...
  const validate = () => {
    ...
    const anyAddr =
      ...
      form.address0.line2 ||
      form.address0.city ||
      form.address0.stateCode ||
      form.address0.zip;
    if (anyAddr && !form.address0.line1.trim()) e["address0.line1"] = "Address line 1 is required";

    // If stateCode is present, basic sanity: 2-3 uppercase letters
    if (form.address0.stateCode && !/^([A-Z]{2,3})$/.test(form.address0.stateCode.trim().toUpperCase())) {
      e["address0.stateCode"] = "Use state code (e.g., MH)";
    }
  ...
  const handleSubmit = async (ev: React.FormEvent) => {
    ...
    // Normalize stateCode to uppercase if provided
    const stateCode = form.address0.stateCode?.trim() ? form.address0.stateCode.trim().toUpperCase() : undefined;

    // Payload matches backend expectation: addresses[].stateCode
    const payload: Partial<Customer> & { addresses?: Array<{
      line1: string;
      line2?: string;
      city?: string;
      zip?: string;
      stateCode?: string; // <-- key field for relation mapping in backend
    }> } = {
      name: form.name.trim(),
        ...
        ? [
            {
              ...
              stateCode, // send only if present
            },
          ]
        : [],
    };
    ...
    try {
      ...
      const msg =
        anyErr?.response?.data?.message || anyErr?.message || "Failed to save customer";
      setGlobalError(msg);
    } finally {
      setSaving(false);
    }
      ...
      >
        ...
        <div className="rounded-xl border border-white/20 bg-white/5 p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            />
            <input
              value={form.address0.stateCode}
              onChange={(e) => setAddr("stateCode", e.target.value.toUpperCase())}
              placeholder="State code (e.g., MH)"
              className={`rounded-lg px-3 py-2 bg-white text-gray-900 placeholder-gray-400 border 
                         ${errors["address0.stateCode"] ? "border-red-400 ring-2 ring-red-300" : "border-white/30 focus:ring-2 focus:ring-blue-300"}
                         focus:outline-none`}
            />
            <input
