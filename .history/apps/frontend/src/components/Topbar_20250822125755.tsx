...
function getActiveTenantName(): string | null {
  // optional future: if you store a friendly name, we’ll show it
...
export default function Topbar({ toggleSidebar }: TopbarProps) {
  ...
  // UX: if you don’t want to see raw "1", hide numeric-only default unless a friendly name exists
  ...
  const onLogout = () => {
    try {
      if (typeof doLogout === "function") {
        doLogout(); // removes token, activeTenantId, currentUser (your service)
      } else {
      ...
      router.push("/loginpage"); // fix: consistent route casing
    }
  };
            ...
            {accountOpen && (
              ...
              >
                ...
                <button
                  onClick={goSettings}
                  className="flex items-center gap-2 w-full px-4 py-2 text-left hover:bg-white/60 dark:hover:bg-gray-800/60"
                  // optional: hide this if not admin
