...
export default function RevenueTrend({ data, loading }: { data?: Revenue7d | null; loading?: boolean }) {
  ...
  const total = series.reduce<number>((acc, cur) => acc + cur.rooms + cur.pos, 0);
      ...
      <div className="h-72">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={series} margin={{ top: 6, right: 12, left: -8, bottom: 2 }}>
            ...
            <YAxis tickLine={false} axisLine={false} width={60} tickFormatter={(v: number) => inrCompact(v).replace("₹", "₹ ")} />
            <Tooltip
              formatter={(val: number, n: string) => [inr(Number(val)), n === "rooms" ? "Rooms" : "POS"]}
