...
import { getCurrentUser, refreshPermissionsForActiveTenant, clearPermissions } from "@/lib/auth";
...
export default function Topbar({ toggleSidebar }: TopbarProps) {
  ...
  // keep local state in sync with LS changes
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (fallback != null) {
      ...
      localStorage.setItem("tenantId", t); // keep legacy in sync
      setActiveTenantId(t);
      // hydrate permissions for first tenant
      refreshPermissionsForActiveTenant().catch(() => {});
    }
  }, [user]);

  // 🔄 When tenant changes (in this tab or another), refresh effective permissions
  useEffect(() => {
    if (!activeTenantId) return;
    refreshPermissionsForActiveTenant()
      .then(() => {
        // let UI react immediately (Sidebar listens to this custom event too)
  ...
  const onLogout = () => {
    try {
      if (typeof doLogout === "function") {
        ...
        clearPermissions(); // make sure perms are not leaked to next session
      }
    } finally {
  ...
  /* ------------------------------ SEARCH BAR --------------------------------
     ...
     - Enter on "#123" or number → Bookings (by ref/ID)
     - "room 101" / "rm 101" → Rooms search
     - Keywords route to modules; otherwise → Customers search
     - Emits CustomEvent("app:search", { detail: { query } })
  --------------------------------------------------------------------------- */
  ...
  // window bridge to push notifications
  useEffect(() => {
    if (typeof window === "undefined") return;
    const api = {
              ...
              >
                ...
                <button
                  onClick={() => {
                    ...
                    router.push("/settings/profile"); // Profile under settings
                  }}
                ...
                <button
                  onClick={() => {
                    setAccountOpen(false);
                    router.push("/settings/userSettings"); // ✅ redirect to /settings/userSettings
                  }}
                  className="flex items-center gap-2 w-full px-4 py-2 text-left hover:bg-white/60 dark:hover:bg-gray-800/60"
                >
